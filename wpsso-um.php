...
if ( ! class_exists( 'WpssoUm' ) ) {
	...
	class WpssoUm {
		...
		public static function required_notice( $deactivate = false ) {
			...
			$error_msg = __( 'The %1$s extension requires the %2$s plugin &mdash; install and activate the %3$s plugin or <a href="%4$s">deactivate the %5$s extension</a>.', 'wpsso-um' );
		...
		private function min_version_notice() {
			...
			if ( method_exists( 'SucomUtil', 'safe_trigger_error' ) ) {
				...
				$error_prefix = sprintf( __( '%s warning:', 'wpsso-um' ), $info['short'] );
