<?php die( 'These aren\'t the droids you\'re looking for...' );
// extract of translatable text strings from common library classes
__( 'An inconsistency was found in the %s update server address &mdash; the network address provided by the local host does not match the DNS network address.', 'wpsso-um' );
__( 'Failed saving retrieved update information for %s.', 'wpsso-um' );
__( 'Please note that one or more non-stable / development %s have been selected.', 'wpsso-um' );
__( 'The %1$s plugin (%2$s) version number is missing from the WordPress plugins array.', 'wpsso-um' );
__( 'Update checks for %s have been disabled while this inconsistency persists.', 'wpsso-um' );
__( 'Update error from the WordPress wp_remote_get() function: %s', 'wpsso-um' );
__( 'Update information for %s has been retrieved and saved.', 'wpsso-um' );
__( 'WordPress function %s is missing and required.', 'wpsso-um' );
__( 'WordPress library file %s is missing and required.', 'wpsso-um' );
_x( 'Update Version Filters', 'metabox title', 'wpsso-um' );
__( 'You may <a href="%s">open a new support ticket</a> if you believe this error message is incorrect or inaccurate.', 'wpsso-um' );
